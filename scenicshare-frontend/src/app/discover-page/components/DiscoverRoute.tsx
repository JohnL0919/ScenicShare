...
export default function DiscoverRoute() {
    ...
    const fetchRoutes = async () => {
      try {
        const { routes: fetchedRoutes, lastDoc: newLastDoc } =
          await getAllRoutes(12);
        setRoutes(fetchedRoutes);
        setLastDoc(newLastDoc);
        setHasMore(fetchedRoutes.length === 12);
      } catch (error) {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      setLastDoc(newLastDoc);
      setHasMore(moreRoutes.length === 12);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="mt-8 px-4 sm:px-6 lg:px-8 lg:ml-20 flex justify-center items-center min-h-[400px]">
      ...
      {hasMore && (
        <div className="flex justify-center mt-12">
          <button
            ...
            className="bg-white/90 hover:bg-white text-gray-900 font-semibold py-3 px-8 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loadingMore ? (
